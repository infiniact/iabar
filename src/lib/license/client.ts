...
async function loadCache(): Promise<LicenseCache> {
  ...
  const cache: LicenseCache = {
    deviceUuid: stored?.deviceUuid || crypto.randomUUID(),
    fingerprint: stored?.fingerprint || crypto.randomUUID().replace(/-/g, ''),
    licenseKey: stored?.licenseKey,
    token: stored?.token,
    maxIssuedAt: stored?.maxIssuedAt ?? 0,
    pubkeys: stored?.pubkeys,
  }
  // Persist freshly-minted identity so it's stable across launches.
  if (!stored?.deviceUuid || !stored?.fingerprint) await saveCache(cache)
  return cache
}
